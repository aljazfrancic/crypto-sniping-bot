...
class SniperBot:
    ...
    def _setup_web3(self):
        ...
        if self.config.RPC_URL.startswith('ws'):
            provider = Web3.WebsocketProvider(
                ...
                websocket_kwargs={'max_size': 1024 * 1024 * 10}  # 10MB max message size
            )
        else:
    ...
    async def _is_token_safe(self, token_address, pair_address):
        ...
        try:
            ...
            if liquidity_eth < self.config.MIN_LIQUIDITY:
                logger.warning(f"Insufficient liquidity: {liquidity_eth:.4f} ETH < {self.config.MIN_LIQUIDITY} ETH")
                return False
    ...
    async def _execute_buy(self, token_address, pair_address, is_token0):
        ...
        try:
            ...
            logger.info(f"Attempting to buy ~{expected_tokens:.2f} tokens with {self.config.BUY_AMOUNT} ETH")
        ...
        while self.running and position['status'] == 'bought':
            try:
                ...
                if price_change >= self.config.PROFIT_TARGET:
                    logger.info(f"Profit target reached for {token_address}: {price_change:.2f}%")
                    await self._execute_sell(token_address, "profit_target")
                    break
                    
                # Check stop loss
                elif price_change <= -self.config.STOP_LOSS:
                    logger.warning(f"Stop loss triggered for {token_address}: {price_change:.2f}%")
    ...
    async def _execute_sell(self, token_address, reason):
        ...
        try:
            ...
            tx_hash = await self.trading.sell_token(
                token_address,
                balance,
                0  # Will use slippage calculation in contract
            )
    ...
    async def display_stats(self):
        ...
        while self.running:
            try:
                ...
                bought = sum(1 for p in self.positions.values() if p['status'] == 'bought')
                sold = sum(1 for p in self.positions.values() if p['status'] == 'sold')
                
                logger.info(f"Bot Statistics - Total: {total_positions}, Active: {bought}, Sold: {sold}")
