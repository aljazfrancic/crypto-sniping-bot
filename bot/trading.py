...
class TradingEngine:
    ...
    def _init_contracts(self) -> None:
        ...
        self.router = self.blockchain.w3.eth.contract(
            address=self.config.router_address, abi=router_abi
        )

        # Factory contract
        factory_abi = self.config.get_abi("factory")
        self.factory = self.blockchain.w3.eth.contract(
            address=self.config.factory_address, abi=factory_abi
        )

        # WETH contract (uses ERC20 ABI)
        weth_abi = self.config.get_abi("erc20")
        self.weth = self.blockchain.w3.eth.contract(
            address=self.config.weth_address, abi=weth_abi
        )

    def buy_token(
        self, token_address: str, amount_eth: float, slippage: Optional[float] = None
    ) -> str:
        ...
        try:
            ...
            amount_out_min = self._calculate_min_output(
                token_address, amount_wei, slippage
            )
            ...
            raise TradingError(f"Invalid input value: {str(e)}")
        except TimeoutError as e:
            raise TradingError(f"Transaction timeout: {str(e)}")
        except ConnectionError as e:
            raise TradingError(f"Network connection error: {str(e)}")
        except Exception as e:
            logger.error(f"Unexpected trading error: {str(e)}")
            raise TradingError(f"Unexpected error: {str(e)}")

    def _calculate_min_output(
        self, token_address: str, amount_wei: int, slippage: Optional[float] = None
    ) -> int:
        ...
        amounts_out = self.router.functions.getAmountsOut(
            amount_wei, [self.config.weth_address, token_address]
        ).call()
    ...
    def _get_expected_price(self, token_address: str, amount_wei: int) -> float:
    ...
    def _simulate_buy(
        self, token_address: str, amount_wei: int, amount_out_min: int
    ) -> None:
    ...
    def _estimate_gas(
        ...
        try:
            return self.router.functions.swapExactETHForTokens(
                ...
                {"from": self.blockchain.account.address, "value": amount_wei}
            )
        except Exception as e:
    ...
    def get_position(self, token_address: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get token balance
            balance = token.functions.balanceOf(self.blockchain.account.address).call()
    ...
    def _get_token_price(self, token_address: str) -> float:
        ...
        try:
            ...
            pair = self.blockchain.w3.eth.contract(address=pair_address, abi=pair_abi)
