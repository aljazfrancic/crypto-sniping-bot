...
describe("Sniper Contract", function () {
  ...
  describe("Buy Token", function () {
    it("Should buy tokens successfully", async function () {
      const { sniper, token, tokenAddress } = await loadFixture(
    ...
    it("Should revert if no ETH sent", async function () {
      const { sniper, tokenAddress } = await loadFixture(
        deployContractsFixture,
      );
      await expect(sniper.buyToken(tokenAddress, 0)).to.be.revertedWith(
        "No ETH sent",
      );
    });
    ...
    it("Should only allow owner to buy", async function () {
      const { sniper, tokenAddress, user1 } = await loadFixture(
        deployContractsFixture,
      );
      await expect(
        sniper
          .connect(user1)
          .buyToken(tokenAddress, 0, { value: ethers.parseEther("1") }),
  ...
  describe("Sell Token", function () {
    it("Should sell tokens successfully", async function () {
      ...
      await expect(sniper.sellToken(tokenAddress, tokenBalance)).to.emit(
        sniper,
        "TokenSold",
      );
      const ethBalance = await ethers.provider.getBalance(
        await sniper.getAddress(),
      );
  ...
  describe("Configuration", function () {
    ...
    it("Should not allow slippage over 50%", async function () {
      const { sniper } = await loadFixture(deployContractsFixture);
      await expect(sniper.updateSlippage(5001)).to.be.revertedWith(
        "Slippage too high",
      );
    });
  ...
  describe("Withdrawals", function () {
    ...
    it("Should withdraw tokens", async function () {
      const { sniper, token, tokenAddress, owner } = await loadFixture(
        deployContractsFixture,
      );
      await sniper.buyToken(tokenAddress, 0, { value: ethers.parseEther("1") });
    ...
    it("Should only allow owner to withdraw", async function () {
      ...
      await expect(sniper.connect(user1).withdrawETH()).to.be.revertedWith(
  ...
  describe("Pair Info", function () {
    it("Should get pair info", async function () {
      const { sniper, tokenAddress, pairAddress } = await loadFixture(
        deployContractsFixture,
      );
      const [pairAddr, tokenReserve, wethReserve, price] =
        await sniper.getPairInfo(tokenAddress);
